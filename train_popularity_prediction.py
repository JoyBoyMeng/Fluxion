        ...
        for epoch in range(args.num_epochs):
            ...
            for batch_idx, train_data_indices in enumerate(train_idx_data_loader_tqdm):
                ...
                if sum(batch_labels) != 0:
                    ...
                    if global_step == 0:  # 只在第一次做验证
                        # 用断言证明“需要训练的参数都收到了梯度”，由于三个bank后面才detach，所以切片赋值导致他们带梯度，去掉.
                        no_grad_names = [n for n, p in model.named_parameters()
                                         if p.requires_grad and p.grad is None and 'bank' not in n]
