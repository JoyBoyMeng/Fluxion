...
class Data:
    ...
    def __init__(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray,
                 edge_ids: np.ndarray, labels: np.ndarray, slow_features: np.ndarray):
        """
...
def get_popularity_prediction_data(dataset_name: str, val_ratio: float, test_ratio: float, time_interval: str):
    ...
    if dataset_name == 'aminer':
        ...
        graph_df = pd.read_csv('./processed_data/{}/pp_{}_{}.csv'.format(dataset_name, dataset_name, time_interval))
        NODE_FEAT_DIM = EDGE_FEAT_DIM = 172
        src_node_ids = graph_df.u.values.astype(np.longlong)
        dst_node_ids = graph_df.i.values.astype(np.longlong)
        edge_ids = graph_df.idx.values.astype(np.longlong)
        unique_node_ids = set(src_node_ids) | set(dst_node_ids)
        unique_node_ids_num = len(unique_node_ids) + 1
        edge_ids_num = max(edge_ids) + 1
        node_raw_features = np.zeros((unique_node_ids_num, NODE_FEAT_DIM))
        edge_raw_features = np.zeros((edge_ids_num, EDGE_FEAT_DIM))
    elif dataset_name == 'aminer backup':
        edge_raw_features = np.load('./processed_data/{}/{}_edgefeature.npy'.format(dataset_name, dataset_name))
        node_raw_features = np.load('./processed_data/{}/{}_nodefeature.npy'.format(dataset_name, dataset_name))
        NODE_FEAT_DIM = EDGE_FEAT_DIM = 172
        edge_raw_features = np.zeros((edge_raw_features.shape[0], EDGE_FEAT_DIM))
        if node_raw_features.shape[1] < NODE_FEAT_DIM:
            node_zero_padding = np.zeros((node_raw_features.shape[0], NODE_FEAT_DIM - node_raw_features.shape[1]))
            node_raw_features = np.concatenate([node_raw_features, node_zero_padding], axis=1)
        if edge_raw_features.shape[1] < EDGE_FEAT_DIM:
